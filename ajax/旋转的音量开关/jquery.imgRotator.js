			...
			el.mousedown(function (evt)
				{
					...
					var x = (evt.pageX - currOffset.left) - (elWidth / 2);
					var y = (elHeight / 2) - (evt.pageY - currOffset.top);
					
					var dist = Math.sqrt(Math.pow(x-pos_x,2) + Math.pow(y-pos_y,2));
					if(dist <= 10)
					{
						dragging = true;
			...
			el.mousemove(function (evt)
				{
					if(dragging)
					{
						...
						var diff = dragStartAngle - Math.atan2(y, x);
						...
						if(Math.abs(self.rotation) < 5 * Math.PI / 6)
						{
							self.layout();
							opts.drag(self.rotation);
							
							pos_x = 8 * Math.sin(self.rotation);
							pos_y = 8 * Math.cos(self.rotation);

							SetVolume(Math.round(100 * (self.rotation * 6 + Math.PI * 5) / (Math.PI * 10)));
						}
