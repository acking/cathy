...
var TrimPath = {
	g_Templet : {},
	createTemplet : function(tId) {
		var templetElem = document.getElementById(tId);
		var templetText = /^(textarea|input)$/.test( templetElem.tagName.toLowerCase() ) ? templetElem.value : templetElem.innerHTML;
	    var reg = /\{(\w+)\x20begin\}((.|\n)+?)\{end\}/ig;
	    var arr;
	    while ((arr = reg.exec(templetText)) != null) {
	       TrimPath.g_Templet[arr[1]] = arr[2];
	    }
	}
};
